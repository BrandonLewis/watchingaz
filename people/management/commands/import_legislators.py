...
PEOPLE_INDEX = 'http://openstates.sunlightlabs.com/api/v1/legislators/?term=%s&apikey=%s'
PERSON_URL = 'http://openstates.sunlightlabs.com/api/v1/legislators/%s/?apikey=%s'
...
def import_person(person):
    ...
    new_person.active = person['active']
    # these might be empty if the legislator is inactive
    new_person.district = person.get('district', '')
    new_person.party = person.get('party', '')
    new_person.chamber = person.get('chamber', '')
    ...
    for term in person['old_roles']:
        for role in person['old_roles'][term]:
            ...
            p_role, c = Role.objects.get_or_create(person=new_person, **converted)
    ...
    for role in person['roles']:
        ...
        if c:
            updated = True
    #if person['active'] == True: # this is unnecessary!
    ...
    if updated:
        try:
            tracker = Tracker.objects.get(content_type__name='person',
                                          tracked_id=new_person.id)
            tracker.updated = datetime.datetime.now()
            tracker.save()
        except ObjectDoesNotExist:
            pass
    
def import_legislators(state, term, initial=False):
    ...
    if initial:
        ...
        leg_ids = Person.objects.filter(roles__term=term,
                                        roles__type="member").values_list('leg_id')
        for leg_id in sorted(leg_ids, key=lambda x: int(x[0][3:])):
            page = urllib2.urlopen(PERSON_URL % (leg_id[0], settings.SUNLIGHT_API_KEY))
