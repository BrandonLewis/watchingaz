...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Remove the ManyToMany relationship to the ActionType model."
        for action in orm.BillAction.objects.all():
            action.atype = action.type.get().type
            action.save()

    def backwards(self, orm):
        "Add the ManyToMany relationship from ActionType to BillAction."
        for action in orm.BillAction.objects.all():
            action.type = orm['bills.ActionType'].get(type=action.atype)
            action.save()
